    ...
    class Program
    {
        ...
        static void GetTetriminoWave()
        {
            ...
            {   //先頭の文字を切り替えて処理する
                ...
                {　　//先頭の文字を切り替えて処理する
                    List<char> l2 = new List<char>(l1); //残りの文字のリストl1からl2を作る
        ...
        static void GetTetriminoWave2()
        {
            ...
            getTable = GetPermutation<char>(tl2);
            for (int i = 0; i < minoTable.Length; i++)
            {
        ...
        static void GetTetriminoWave3()
        {
            ...
            List<char> tl2 = new List<char>(t);
            List<List<char>> getTable = new List<List<char>>();
            getTable = GetPermutation<char>(new List<char>(), tl2);
            for (int i = 0; i < minoTable.Length; i++)
            {
                minoTable[i] = getTable[random.Next(0, minoPattern.Count)].ToString();
            }
        ...
        static private List<List<T>> GetPermutation<T>(List<T> inList)
        {
            ...
            else
            {
                for (int i = 0; i < inList.Count; i++)
                {
                    ...
                    T topItem = newList[i];
                    newList.RemoveAt(i);
                    getList = GetPermutation(newList);
        ...
        static private List<List<T>> GetPermutation<T>(List<T> fixedList, List<T> inList)
        {
            List<List<T>> val = new List<List<T>>();
            ...
            for (int i = 0; i < inList.Count; i++)
            {
                ...
                if (newList.Count > 0)
                {
                    val.AddRange(GetPermutation<T>(newFixedList, newList));
                }
                else
                {
                    val.Add(fixedList);
